...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (!chatTrigger) {
        console.error("AI Assistant trigger not found!");
        return;
    }

    console.log("AI Assistant initialized for " + (document.body.dataset.userRole || "USER"));
    ...
    function addMessage(text, role, time, type = null, category = null) {
        const div = document.createElement('div');
        div.className = `message ${role} ai-section`;

        // Add Category Attribute if present (For scoped CSS)
        if (category) {
        ...
        if (type) {
            ...
            switch (type) {
                ...
                case 'AI_Legal_Gen': typeLabel = 'AI कायदेशीर सल्ला'; typeClass = 'tag-approved'; break;
        ...
        // Add 'ai-content' class for scoped text styling
        div.innerHTML = `
            ${header}
            <div class="message-content ai-content">${text.replace(/\n/g, '<br>')}</div>
